...
public class NumberGuessingGame {
        ...
        try {
            ...
            switch (menuOption) {
                case 1:
                    ...
                    int randomlyNumber = methodChange.randomlyNumber(numRange);
                    methodChange.guessNumber(randomlyNumber);
                    break;
                case 2:
    ...
    public int randomlyNumber(int numRange) {
        Random random = new Random();
        int randomlyNumber = random.nextInt(numRange) + 1;
        return randomlyNumber;
    }

    public void guessNumber(int randomlyNumber) {
        Scanner input = new Scanner(System.in);
        int UserGuess;
        int guess = 0;
        do {
            ...
            guess++;
            if (UserGuess > randomlyNumber) {
                System.out.println("Lower");
            } else if (UserGuess < randomlyNumber) {
                System.out.println("Higher");
            }
        } while (randomlyNumber != UserGuess);
        System.out.println(" ");
        if (guess == 1) {
